...
class Solver:
        ...
        def __init__(self, config, model_train, train_data, val_data, test_data, my_scenario, my_model, my_size, my_ordering):
                ...
                # ordering_1: pitch-ascending ordering
                if ordering == 'ordering_1':
                    print('ordering_1!')
                    ordering_list = range(self.config.timeslice_size)
                    print('ordering_list',ordering_list)
                # ordering_2: variance-ascending ordering
                elif ordering == 'ordering_2':
                    print('ordering_2!')
                    score = np.var(self.train_data,axis=0)
                    ordering_list = list(np.argsort(score))
                    print('ordering_list',ordering_list)
                # ordering_3: variance-descending ordering
                elif ordering == 'ordering_3':
                    print('ordering_3!')
                    score = np.var(self.train_data,axis=0)
                    ordering_list = list(reversed(np.argsort(score)))
                    print('ordering_list',ordering_list)
                else: 
                    raise ValueError('ordering name does not match, wrong ordering?')

                sorting_index = range(len(ordering_list))   
                sorting_dict = dict(zip(ordering_list, sorting_index))
                
                mp = np.arange(0,self.config.timeslice_size)
        ...
        def validation(self,sv,session,variables_val,n_batch_per_epoch,\
                ...
                for constraint_id,constraint in enumerate(validation_constraints):
                    ...
                    unknown_notes = np.setdiff1d(universe,constraint) # unknown notes are sorted in the ordering ordering
                    ...
                    if len(set(constraint))==0:
                        temp_custom_ordering = np.array(unknown_notes_sorted) 
                    else: 
                        temp_custom_ordering = np.concatenate( [constraint_sorted , unknown_notes_sorted] ) 
                    print('temp_custom_ordering: known and unknown notes should be sorted!',temp_custom_ordering)
                ...
                for epoch_id in range(self.config.n_epochs):
                        ...
                        for i in range(n_batch_per_epoch):
                                ...
                                if model=='model_1':
                                    ...
                                    # order the first d-1 elements by the ordering ordering
                                    # order the last timeslice-d+1 elements by the ordering ordering
                                    lower = non_ordered[:,:d]
                                    upper = non_ordered[:,d:]
                                    
                                    lower_2 = mp[lower]
                                    upper_2 = mp[upper]
                                    lower_indices_to_sort = np.argsort(lower_2)
                                    upper_indices_to_sort = np.argsort(upper_2)

                                    lower_sorted = lower[np.arange(lower.shape[0])[:,np.newaxis],lower_indices_to_sort]
                                    upper_sorted = upper[np.arange(upper.shape[0])[:,np.newaxis],upper_indices_to_sort]
