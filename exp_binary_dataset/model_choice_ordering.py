...
class Model:
	...
	def get_log_prob_op(self, training=False):
		...
		with tf.variable_scope("OrderlessNADE_step"):
			...
			row_indices = tf.py_func(self.my_func_3, [ordering], tf.int32)
			row_indices_b = tf.py_func(self.my_func_4, [ordering], tf.int32)
			...
			if size=='large':
				...
				print('d',d)
				print('tf.reshape(d, [1,1])',tf.reshape(d, [1,1]))

                
				print('tf.shape(inputs_flat)[0]',tf.shape(inputs_flat)[0])
			...
			def body(log_probability_train,i, start_i, reshaped_start_i):
				...
				#V_o_d = V[:,o_d[0]]
				#V_o_d = tf.reshape(V_o_d,[-1,1])
				#temp_product_alternative = tf.matmul(hi,V_o_d)
				#temp_product_alternative = tf.reshape(temp_product_alternative,[-1,])
				#p_o_d_alternative=tf.sigmoid(temp_b + temp_product_alternative)
				#v_o_d_alternative = tf.gather_nd(inputs_flat, coords)
		...
		print('check that index_after_loop corresponds to what we expect!')
		...
		with tf.variable_scope("OrderlessNADE_step"):
			...
			row_indices_val = tf.py_func(self.my_func_3, [my_ordering], tf.int32)
			row_indices_b_val = tf.py_func(self.my_func_4, [my_ordering], tf.int32)
