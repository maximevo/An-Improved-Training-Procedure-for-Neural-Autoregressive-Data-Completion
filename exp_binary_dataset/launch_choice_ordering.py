...
from model_choice_ordering import Model

from solver_choice_ordering import Solver
...
# orderings = ['ordering_1','ordering_2','ordering_3']
orderings = ['ordering_2','ordering_1','ordering_3']
        ...
        for config in config_list:
            for my_scenario in scenarios:
                for my_model in models:
                    for my_ordering in orderings:
                        xp_to_launch.append( (Model,dataset,config, Solver, my_scenario, my_model, my_size,my_ordering ) )
...
def launch_one_experiment(model,dataset,config, my_solver, my_scenario, my_model, my_size,my_ordering):
    ...
    # Instantiate solver
    solver = my_solver(config, model_train, train_X, valid_X, test_X, my_scenario = my_scenario, my_model = my_model, my_size = my_size, my_ordering=my_ordering)
...
def main(): 
    for xp_id, (model,dataset,config, solver, my_scenario, my_model, my_size,my_ordering) in enumerate(xp_to_launch):
        print('===== Launching experiment '+ str(xp_id+1) + " =====")

        model_name = str(model).split('.')[-1]
        print('model: ', model_name, my_model, 'dataset: ',dataset,'config: ',config,'ordering: ',my_ordering)
        launch_one_experiment(model,dataset,config, solver, my_scenario, my_model, my_size,my_ordering)
